...
function getFieldTranslation(field, lang) {
  ...
  if (field) {
    var _field$split = field.split('.'),
        head = _field$split[0],
        tail = _field$split.slice(1);

    if (tail.length > 0) {
      var tailString = tail.join(".");
      var headTranslation = getFieldTranslation(head);
      var tailTranslation = getFieldTranslation(tailString);

      if (headTranslation !== head && tailTranslation !== tailString) {
        var translation = headTranslation !== head && [headTranslation, tailTranslation].filter(function (t) {
          return !!t;
        }).join("->");
